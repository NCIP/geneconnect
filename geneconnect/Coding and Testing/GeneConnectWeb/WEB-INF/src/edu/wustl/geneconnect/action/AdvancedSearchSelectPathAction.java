...
public class AdvancedSearchSelectPathAction extends Action
{
			...
			HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		if (targetAction.equals("filterPaths"))
		{
			...
			//Search operation.
			Logger.out.debug("Target Action is filterPaths" + sourcesMap.size()
					+ " selectedPaths-->" + advancedSearchActionForm.getSelectedPaths());

			Logger.out.debug("Filer Starts With->"
					+ advancedSearchActionForm.getStartsWithDataSources() + " Ends With->"
					+ advancedSearchActionForm.getEndsWithDataSources() + "ONT Filter Code->"
					+ advancedSearchActionForm.getOntFilterCode() + " PathType->"
					+ advancedSearchActionForm.getPathTypes());
			...
			BizLogicInterface ontBizLogic = geneConnectBizLogicFactory
					.getBizLogic(GCConstants.ONT_BIZLOGIC);
			...
			data.put(GCConstants.ONT_FILTER_CODE, new Integer(advancedSearchActionForm
					.getOntFilterCode()));
			data.put(GCConstants.PATH_TYPE, advancedSearchActionForm.getPathTypes());
			data.put(GCConstants.STARTS_WITH_DATA_SOURCE, advancedSearchActionForm
					.getStartsWithDataSources());
			data.put(GCConstants.ENDS_WITH_DATA_SOURCE, advancedSearchActionForm
					.getEndsWithDataSources());
			...
			request.setAttribute(GCConstants.VALID_PATHS_LIST_FOR_DATA_SOURCES,
					filteredPathsForDataSources);
			...
			request.setAttribute(GCConstants.DATA_SOURCES_LIST, session
					.getAttribute(GCConstants.DATA_SOURCES_LIST));
		...
		else
		{
			...
			if (pagination == null || pagination.equals("false"))
			{
				...
				Logger.out.debug("ONT Filter Code at getAllValidPathsForDataSources()->"
						+ advancedSearchActionForm.getOntFilterCode());
				Logger.out.debug("PathType at getAllValidPathsForDataSources()->"
						+ advancedSearchActionForm.getPathTypes());
				...
				session.setAttribute(GCConstants.VALID_PATHS_LIST_FOR_DATA_SOURCES,
						validPathsForDataSources);
			}
			//setting list of DataSources into request object
			request.setAttribute(GCConstants.DATA_SOURCES_LIST, session
		...
		if (advancedSearchActionForm.isBackFromSelectPath())
		{
			...
			Logger.out.debug("Initial InputOutput-->"
					+ advancedSearchActionForm.getInitialInputOutput());
			...
			{
				Logger.out
						.debug("!!!!!!!!!!!!!!!!!!!!!!! Same InputOutput !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
				//setting already selected paths by user into request object to display on JSP page
				request.setAttribute(GCConstants.ALREADY_SELECTED_PATHS,
						setAlreadySelectedPaths(advancedSearchActionForm.getSelectedPaths()));
			}
		}
		...
		if (alreadySelectedPaths != null)
		{
			...
			{
				request.setAttribute(GCConstants.ALREADY_SELECTED_PATHS,
						setAlreadySelectedPaths(alreadySelectedPaths));
			}
		}
	...
	private List createListOfDataSources(ActionForm form, HttpServletRequest request)
			throws Exception
	{
		//initializing received formbean object
		AdvancedSearchForm advancedSearchActionForm = (AdvancedSearchForm) form;
		...
		for (int i = 0; i < keysList.size(); i++)
		{
			...
			if ((sourcesMap.get((String) keysList.get(i)) != null)
					& !(sourcesMap.get((String) keysList.get(i)).equals("")))
			{
				...
				Logger.out.debug("InputDataSource entered on AdvanceSearch page:- "
						+ dataSourceName + " id->" + id);
			}
		}
		...
		for (int i = 0; i < dataSources.size(); i++)
		{
			try
			{
				...
				if ((request.getParameter(bean.getName()) != null))
				{
					...
					Logger.out.debug("OutputDataSource entered on AdvanceSearch page:- "
							+ bean.getName());
		...
		{
			enteredDataSourcesList.add(new NameValueBean(enteredDataSourcesKeyList.get(i),
					enteredDataSources.get(enteredDataSourcesKeyList.get(i))));
		}
	...
	private Map setAlreadySelectedPaths(String alreadySelectedPaths)
	{
		...
		while (alreadySelectedPathsTokenized.hasMoreTokens())
		{
			...
			String alreadySelectedPathValue = alreadySelectedPathName.substring(
					(alreadySelectedPathName.indexOf("=") + 1), alreadySelectedPathName.length());

			//fetching name of the path from tokened path
			alreadySelectedPathName = alreadySelectedPathName.substring(0, alreadySelectedPathName
					.indexOf("="));
			...
			Logger.out.debug("PathName==>" + alreadySelectedPathName + " PathValue==>"
					+ alreadySelectedPathValue);
		}
