...
sub spawn_jobs
{
    ...
    #open(QMGR_LOG_FILE, '>>QueueManagerLog.txt'  ) or die "Error opening QueueManagerLog.txt.";

   # print "\n\nQueue Manager - Spawning jobs... ";
    ...
    {
#	print "\n *******\nNo. of Jobs in the queue : ",  $jobQ->get_queue_size;
 #       print "\nAvailable Machines > " ;

	my @machines_avail = $machineQ->get_available_machines ;
#	print "Machine Names : " , @machines_avail;
	my $count=0;	
	while($count < scalar $machineQ->get_available_machines ) {
  #           print " " , ($machines_avail[$count])->get_name() , " \t";
		$count++;
	}
        ...
        if( defined $next_job )
        {
		    ...
		    print "\n\nExecuting job : ", $next_job->get_cmd_line, " on machine  ", $mach_obj->get_name(), "\n";
        	}
        	else
        	{
        		$self->queue_job( $next_job );
                   # print "\n\nQueueing job  : ", $next_job->get_cmd_line;
        	}
        }
    }
#	print  "\nQueue Manager - Finished  Spawning jobs... ";
#	close(QMGR_LOG_FILE);
	return 1;
}
