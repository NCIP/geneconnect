...
print WRITE "LOAD DATA INFILE * APPEND INTO TABLE UNIPROT_REFSEQPROTEIN_U FIELDS TERMINATED BY '###' (URE_UNIPROTKBID, URE_REFSEQPROTEINID, URE_LINKTYPE)" . "\n" . "BEGINDATA" . "\n";

open(WRITE2,">$configParams{BASEDIR}/refseq_uniprot.txt");

print WRITE2 "LOAD DATA INFILE * APPEND INTO TABLE REFSEQPROTEIN_UNIPROT_U FIELDS TERMINATED BY '###' (REU_REFSEQPROTEINID, REU_UNIPROTKBID, REU_LINKTYPE)" . "\n" . "BEGINDATA" . "\n";
my $temp;
my @refseq;
